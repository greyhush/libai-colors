...
from wangwei_data import get_poems, get_color_rgb, get_annotations, get_color_meanings
...
def generate_color_bar(output_path="wangwei_color_of_life.png"):
    poems = get_poems()
    ...
    # ── 创建画布 ──
    fig = plt.figure(figsize=(22, 16), facecolor='#0d1b2a')
    ...
    ax_stage.set_facecolor('#0d1b2a')
    ax_annot.set_facecolor('#0d1b2a')
    ax_legend.set_facecolor('#0d1b2a')
    ...
    for i, poem in enumerate(poems):
        ...
        if not colors:
            ...
            ax_bar.add_patch(mpatches.FancyBboxPatch(
                ...
                facecolor='#2d3a4a', edgecolor='none', alpha=0.6
            ))
            continue
    ...
    # ── 年龄标记 ──
    for age in range(15, 65, 5):
        ax_bar.axvline(x=age, color='#3d5a80', linewidth=0.5, linestyle='--', alpha=0.5)
        ax_bar.text(age, -0.35, f"{age}岁", ha='center', va='top',
                    fontproperties=font_prop, color='#8899aa', fontsize=9)

    ax_bar.set_xlim(min_age, max_age)
    ax_bar.set_ylim(-0.6, 1.5)
    ax_bar.set_title('王维的人生颜色  The Color of Wang Wei\'s Life (701-761)',
